...
class CTkWindow(CTk):
    def __init__(self,
                 geometry="500x300",  # Application name
                 app_title="Azalea - Register",  # Enter window geometry
        ...
        self.header = CTkFrame(self, corner_radius=round_corner, fg_color=self.titlebar_color,
        ...
        fg_color = ThemeManager.theme["CTk"]["fg_color"] if fg_color == "default" else fg_color

        self.app = CTkFrame(self, corner_radius=round_corner, bg_color=transparent_color, fg_color=fg_color,
        ...
        self.ctkimage = CTkImage(Image.open(
            os.path.join(os.path.dirname(customtkinter.__file__), "assets", "icons", "CustomTkinter_icon_Windows.ico")),
                                 size=(16, 16))
        self.icon = self.ctkimage if icon is None else CTkImage(Image.open(icon), size=(16, 16))
        self.title_label = CTkLabel(self.header, width=10, image=self.icon, compound="left", text=f"  {app_title}",
                                    anchor="n", text_color=title_color)
        if justify == "center":
        ...
        self.style = style
        if style == "modern":
            self.button_close = CTkButton(self.header, corner_radius=10, width=10, height=10, text="",
            ...
            self.button_max = CTkButton(self.header, corner_radius=10, width=10, height=10, text="",
            ...
            self.button_min = CTkButton(self.header, corner_radius=10, width=10, height=10, text="",
            ...
            self.button_close = CTkButton(self.header, corner_radius=round_corner, width=40, height=30, text="✕",
            ...
            self.button_max = CTkButton(self.header, corner_radius=0, width=40, height=30, text="□",
            ...
            self.button_min = CTkButton(self.header, corner_radius=0, width=40, height=30, text="—",
                                        text_color=["black", "white"],
    ...
    def configure(self, **kwargs):
        ...
        if "icon" in kwargs:
            self.icon = CTkImage(Image.open(kwargs["icon"]), size=(16, 16))
            self.title_label.configure(image=self.icon)
...
class App:
    def __init__(self, window, **kwargs):
        ...
        # Close Button
        close_button = CTkButton(window, text="X", width=25, height=25, corner_radius=0,
                                 command=lambda: close_window(self, window), border_width=1)
        close_button.configure(fg_color="gray", hover_color="red")
        close_button.pack(anchor="ne")

        # Username
        self.label1 = CTkLabel(window, text="Username:")
        self.label1.pack(pady=10)
        self.entry1 = CTkEntry(window, textvariable=self.text1, font=('none', 12, 'italic'))
        self.entry1.pack()

        # Password
        self.label2 = CTkLabel(window, text="Insert password")
        self.label2.pack(pady=10)
        self.entry2 = CTkEntry(window, textvariable=self.text2, show="*")
        self.entry2.pack()

        # Register
        text3 = None
        self.validator_warning = CTkLabel(window, text=text3)
        self.validator_warning.pack(pady=10)
        self.bvalidate = CTkButton(window, text="Register",
                                   command=lambda: self.validator(self.text2, self.validator_warning))
        self.bvalidate.pack(pady=20)

    def check_len(self, label, textvar, text):
        label.configure(text=(str(text) + str(textvar)))

    def change_text(self, label, textvar, text):
        label.configure(text=(str(text) + str(textvar.get())))

    # Validation
    def validator(self, passwd, label):
        if len(passwd.get()) < 7:
            tval = "Your password should have more than 7 characters."
        elif not any(char.isupper() for char in passwd.get()):
            tval = "Your password must contain an uppercase letter."
        elif any(char.isupper() for char in passwd.get()) and any(char.isdigit() for char in passwd.get()):
            tval = f"You've registered as: {self.text1.get()}\nCongratulations."

            with open("LoginBase.txt", "r+") as x:
                if self.text1.get() in x.read():
                    tval = "Username taken."
                    x.close()
                else:
                    with open("LoginBase.txt", "a") as x:
                        x.writelines(f"User: {self.text1.get()}, Pass: {self.text2.get()} \n")
                        x.close()
        else:
            tval = "Your password must contain a number"
        label.configure(text=tval)
